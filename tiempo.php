...
{
    $urlContents = file_get_contents("[https://api.openweathermap.org/data/2.5/weather?q=]".urlencode($_GET['ciudad']).",&appid=[>>> Ingrese su API KEY AQUI <<<]");
    ...
    $previsionTiempo1 = "La temperatura es ".intval($temperaturaEnCelsius)."&deg;C";
    $tempMin = $array['main']['temp_min']-273;
    $tempMax = $array['main']['temp_max']-273;
    $previsionTiempo2 = " Oscilando entre ".intval($tempMin)."&deg;C de minima y ".intval($tempMax)."&deg;C de máxima.";
}
?>
            ...
            <div id="prevision Tiempo">
                <?php
                    if ($previsionTiempo){
                        ...
                        echo'<div class="alert alert-success" role="alert">'.$previsionTiempo1.'</div>';
                        echo'<div class="alert alert-success" role="alert">'.$previsionTiempo2.'</div>';
                    }
                    else if ($error != ""){
